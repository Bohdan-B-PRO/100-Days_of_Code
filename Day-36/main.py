...
news_date_response = requests.get(NEWS_ENDPOINT, params=alpha_vantage_parameters)
news_date_response.raise_for_status()
news_data = alpha_vantage_response.json()

closing_prices = []
for _, data in zip(range(3), stock_data["Time Series (Daily)"]):
    closing_prices.append(stock_data["Time Series (Daily)"][data]["4, close"])
